...
const register = async (req, res) => {
    try {
        ...
        if (role === 'student') {
            ...
            const parentQuery = await mongodb_1.collections.users
        ...
        await mongodb_1.collections.users.doc(userRecord.uid).set(userData);
        ...
        if (role === 'student' && parentDoc) {
            ...
            await mongodb_1.collections.users.doc(parentDoc.id).update({
...
const updateProfile = async (req, res) => {
    try {
        ...
        await mongodb_1.collections.users.doc(req.user.id).update(updates);
        // Get updated user data
        const updatedUserDoc = await mongodb_1.collections.users.doc(req.user.id).get();
        const updatedUser = { ...updatedUserDoc.data(), id: req.user.id };
        res.status(200).json({
            success: true,
            data: {
                user: updatedUser,
